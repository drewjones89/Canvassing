...
canvass <- read_excel("~/PLS Research /Canvassing Project.xlsx")
...
In this project I analyze the a dataset that I create comprised of records of my canvassing in Michigan's 84th State House District during the 2024 election. To create the dataset, I logged the results of my individuals knock lists. 
...
A single address with 2 voters that I was attempting to speak to specifically counts for 2 attempts and 1 door. On this campaign, lists varied from ~30-90 doors, and were usually completed fully. 

My response variable `response_rate` os comprised of 100 $\div$ (Doors $\div$ Responses) to give a percentage of doors that saw a response. 
...
arrange(desc(skim_type)) |>
  select(skim_variable, numeric.hist, everything(), -skim_type, -POSIXct.min, -POSIXct.max, -POSIXct.median, -POSIXct.n_unique, -character.min,
...
### ANOVA: Weekend
Next, I investigate whether `Weekend` is useful in helping to predict *response rate*. 
...
The first condition is normal distribution of residuals, which I examine using the QQ plot, Histogram of residuals, and boxplot of residuals in *figure 7*. The data is left-tailed but not egregiously so. I consider the residual distribution assumption met here. 
...
Next, I examine the residuals vs. predicted value plot in *figure 8*, and see that the residuals do not vary constantly. The residuals appear to increase as the predicted values grow larger. The assumption of constant variance is not met. 
...
The difference in mean `response rate` for the two levels of `Weekend` are not statistically significant.
...
combined_anova_plots_month <- dist_point_month + dist_hist_month + dist_box_month +
  plot_layout(ncol = 3, widths = c(1, 1.2, 1)) +
  plot_annotation(
    caption = "Figure 6: ANOVA assumption plots for Month") 

combined_anova_plots_month
```

For the `Month` model, the normal distribution of residuals assumption appears to be met. There is an outlier point which makes the distribution slightly left tailed but, as with `Weekend`, not severely. 
...
Some clustering is present in *Figure X*. However, looking past the lone outlier with a residual above 20, the constant variance assumption is met. The residuals are relatively constant across the predicted values. 
...
anova_table_month <- anova(lm_month) |>
  flextable() |>
  set_caption((caption = "Figure 8: ANOVA Table for `Month`")) |>
  theme_vanilla()

anova_table_month
```

Based on the P-Value 0.54 I conclude there is not statistically significant evidence that `Month` is useful for predicting the mean `response_rate`
...
canvass_clean |>
  ...
  labs(x = "Steps",
       ...
       caption = "Figure 9: Steps-response_rate") +
    scale_y_continuous(labels = label_percent(scale = 1)) +
  theme_few() +
  theme(  plot.title = element_text(hjust = 0.5),
    panel.grid.major = element_line(color = "gray93"),
...
canvass_clean |>
  ...
  labs(x = "Attempts",
       ...
       caption = "Figure 10: Attempts-response_rate") +
    scale_y_continuous(labels = label_percent(scale = 1)) +
  theme_few() +
  theme(  plot.title = element_text(hjust = 0.5),
    panel.grid.major = element_line(color = "gray93"),
...
In *Figure 10*, there is a strong negative relationship between `Attempts` and `response_rate`. This makes sense, as a the observations with the largest number of attempts are likely households with multiple voters, such as one containing parents and children. A non-response to one of these doors would mean a high number of attempts. 
...
canvass_clean |>
  ...
  labs(x = "Lists",
       ...
       caption = "Figure 11: Lists-response_rate") +
    scale_y_continuous(labels = label_percent(scale = 1)) +
  theme_few() +
  theme(  plot.title = element_text(hjust = 0.5),
...
attempts_list_interaction_plot <- canvass_clean |>
  ...
  labs(x = "Attempts",
       ...
       caption = "Figure 13: Lists-attempts interaction",
       color = "Lists") +
  scale_y_continuous(labels = scales::label_percent(scale = 1)) +
  scale_fill_manual(
    values = c("1" = "#D55E00", 
...
There may be an interaction between `Attempts` and `Lists` for a similar reason. Days with a greater number of `Lists` may have also seen a greater number of `Attempts` simply due to the amount of time spent canvassing. In *Figure 13*, we do appear to see an interaction effect between `attempts` and `lists`. 
...
attempts_weekend_interaction_plot <- canvass_clean |>
  ...
  labs(x = "Attempts",
       ...
       caption = "Figure 14: Weekend-attempts interaction") +
  scale_y_continuous(labels = scales::label_percent(scale = 1)) +
  scale_fill_manual(
    values = c("yes" = "#D55E00", 
               "no" = "#0072B2"),
...
I would anticipate and interaction between `Attempts` and `Weekend` due to weekend days seeing a greater number of average hours worked than weekdays. In *Figure 14*, an apparent interaction between `Attempts` and `Weekend` is observed. 
...
steps_weekend_interaction_plot <- canvass_clean |>
  ...
  labs(x = "Steps",
       ...
       caption = "Figure 15: Weekend-steps interaction",
       color = "Weekend") +
  scale_y_continuous(labels = scales::label_percent(scale = 1)) +
  scale_fill_manual(
    values = c("yes" = "#D55E00", 
...
An interaction between `Steps` and `Weekend` may also be expected for the reason of time worked on a weekend day. In *Figure 15* and interaction between `Steps` and `Weekend` is observed. 
...
steps_attempts_interaction_plot <- canvass_clean |>
  ...
  labs(
    ...
    caption = "Figure 16: `Steps` and `Attempts` interaction"
  ) +
  theme_few() +
  theme(
    plot.title = element_text(hjust = 0.5),
...
`Steps` and `Attempts` may interact as well. Logically, `Attempts` would increase as `Steps` did. However, in *Figure 16*, there does not seem to be an effect, or at least a strong one. *Figure 16* is faceted on a binned version of the `Attempts` variable, and although the regression lines are not parallel, the three facets move in the same direction, and the low number of observations in the High plot leads me to believe that the trend would be similarly positive if more observations were present. Overall, this interaction effect is not clear enough to be included in the model. 

I have determined that the plausible interactions between variables here are `Attempts`\*`Lists`, `Attempts`\*`Weekend`, and `Steps`\*`Weekend`. These interaction will be used to fit the linear regression model. 
...
as_tibble(all_models$result) |>
  ...
  slice_max(adjr, n = 5) |>
  flextable() |>
  set_caption(caption = "Figure 17: Models sorted by Adjusted R-Squared") |>
  theme_vanilla()
```

Fitting all possible models from the variables shows that the 3 term model with `Lists`, the interaction `Weekend`\*`Attempts`, and the interaction `Weekend`\*`Steps` is the best, as can be seen in *Figure 17*. It features identical adjusted r-squared and AIC as bigger models that includes `Attempts` and `Steps`.

I now recreate the chosen model to examine some of the effects closer. Using `lm()` to create it will provide slighly different values than `ols_steps_all_possible()`, which was used to fit all possible models and is the cource of the values in *Figure 17*
...
tidy(chosen_model)|> 
  ...
  flextable() |>
  set_caption(caption = "Figure 18: Model terms") |>
  theme_vanilla()
```

Looking at *Figure 18*, we see that the most powerful piece of the model is `Weekend`, with Weekend days leading to an approximate increase in `response_rate` of 4.8%.




### Appendix: Visual design of this post
